...
def apply_hard_filters(rows: List[Dict[str, Any]], targets: Dict[str, Any], hard_keys: List[str]) -> List[Dict[str, Any]]:
    """硬门槛过滤：只有当每个 hard_key 的 value 落在 [low, high] 内才保留。"""
    if not hard_keys:
        return rows
    kept = []
    for r in rows:
        ...
        for k in hard_keys:
            ...
            if v is None:
                ok = False
                break
            low = safe_float(targets[k].get("low"))
            high = safe_float(targets[k].get("high"))
            if low is None or high is None:
                ok = False
                break
            if not (float(low) <= float(v) <= float(high)):
                ok = False
                break
        if ok:
            kept.append(r)
...
def main():
    ...
    ap.add_argument("--hard-target", action="append", default=[],
                    help="硬门槛 target：要求该指标值必须落在 targets 设定的 [low, high] 内；可重复指定")
    ap.add_argument("--hard-flow", action="store_true",
                    help="快捷：等价于 --hard-target flow_share_county_mean")
    args = ap.parse_args()
    ...
    if args.mode == "sweep":
        ...
        hard_keys = list(args.hard_target) if args.hard_target else []
        if getattr(args, "hard_flow", False):
            hard_keys.append("flow_share_county_mean")
        # 去重但保序
        seen = set()
        hard_keys = [k for k in hard_keys if not (k in seen or seen.add(k))]

        rows_for_topk = rows
        if hard_keys:
            rows_for_topk = apply_hard_filters(rows, targets, hard_keys)
            print(f"[hard-filter] keys={hard_keys}  kept={len(rows_for_topk)}/{len(rows)}")
            if len(rows_for_topk) == 0:
                print("[hard-filter] WARNING: no candidates satisfy the hard constraints; falling back to unfiltered rows.")
                rows_for_topk = rows

        topk = select_topk(rows_for_topk, args.k)
