...
- 生成一个与原来风格接近的 share 轨迹图，并叠加不确定性区间（UI band；浅色10–90%，深色25–75%）
...
def quantiles_over_runs(mat: np.ndarray, qs=None) -> np.ndarray:
    """
    mat: shape (n_runs, T)
    qs: iterable of quantile levels in [0,1], e.g. (0.10, 0.25, 0.50, 0.75, 0.90)
...
def build_quantile_levels(outer=(0.10, 0.90), inner=(0.25, 0.75)):
    lo_o, hi_o = outer
    lo_i, hi_i = inner
    if not (0.0 <= lo_o <= lo_i <= 0.5 <= hi_i <= hi_o <= 1.0):
        raise ValueError(f"Invalid quantile order: outer={outer}, inner={inner}")
    return (lo_o, lo_i, 0.50, hi_i, hi_o)
...
def main():
    ...
    ap.add_argument("--outer", type=float, nargs=2, default=(0.10, 0.90), help="浅色带分位区间 (low high), e.g. --outer 0.10 0.90")
    ap.add_argument("--inner", type=float, nargs=2, default=(0.25, 0.75), help="深色带分位区间 (low high), e.g. --inner 0.25 0.75")
    ...
    # 计算分位数（raw + MA）
    X = np.arange(T)
    qs = build_quantile_levels(outer=tuple(args.outer), inner=tuple(args.inner))
    ...
    # UI bands (基于 RAW：代表真实波动)
    q_lo_o, q_lo_i, q_med, q_hi_i, q_hi_o = 0, 1, 2, 3, 4
    # outer (light): outer_low~outer_high
    ax.fill_between(X, q_county_raw[q_lo_o], q_county_raw[q_hi_o], color=county_color, alpha=0.10, linewidth=0)
    ax.fill_between(X, q_primary_raw[q_lo_o], q_primary_raw[q_hi_o], color=primary_color, alpha=0.10, linewidth=0)
    # inner (dark): inner_low~inner_high
    ax.fill_between(X, q_county_raw[q_lo_i], q_county_raw[q_hi_i], color=county_color, alpha=0.18, linewidth=0)
    ax.fill_between(X, q_primary_raw[q_lo_i], q_primary_raw[q_hi_i], color=primary_color, alpha=0.18, linewidth=0)    # median raw (dashed) + median MA (solid) — keep legend similar to your original
    county_med_raw = q_county_raw[q_med]
    primary_med_raw = q_primary_raw[q_med]

    # 1) 先算 IQR 的“几何中线”（永远在 25-75 正中间）
    county_iqr_mid = 0.5 * (q_county_raw[q_lo_i] + q_county_raw[q_hi_i])
    primary_iqr_mid = 0.5 * (q_primary_raw[q_lo_i] + q_primary_raw[q_hi_i])

    # 2) 视觉混合系数：0=完全用median；1=完全用IQR中线
    #    你想“更居中”就把它调大，比如 0.3 / 0.5 / 0.7
    VISUAL_BLEND = 0.6
    ...
    # 3) 你现在的实线是 MA，所以这里对“视觉中心线”做 MA
    county_center_ma = safe_moving_average(county_center_raw, movavg)
    primary_center_ma = safe_moving_average(primary_center_raw, movavg)

    ax.plot(X, county_med_raw, linestyle="--", linewidth=1.0, color=county_color, alpha=0.30, label="County Hospital (raw)")
    ax.plot(X, county_center_ma, linestyle="-", linewidth=2, color=county_color,
            label=f"County Hospital ({movavg}w MA)")

    ax.plot(X, primary_med_raw, linestyle="--", linewidth=1.0, color=primary_color, alpha=0.30, label="Primary Care (Towns + Large Towns) (raw)")
    ax.plot(X, primary_center_ma, linestyle="-", linewidth=2, color=primary_color,
            label=f"Primary Care (Towns + Large Towns) ({movavg}w MA)")
    ax.set_ylabel("Share")
    ax.set_xlabel("Time step")
    ax.set_ylim(0.0, 0.9)
    ax.grid(True, alpha=0.35)
    ax.set_title(f"S2 — Share with uncertainty bands (n={n} plausible parameter sets from S1)\n"
                 f"Band: 25–75% (dark) + 10–90% UI (light); center line: median")
