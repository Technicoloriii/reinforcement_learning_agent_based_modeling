...
# Style
COUNTY_COLOR = "#2C458D"   # deep blue
PRIMARY_COLOR = "#EF5950"  # red
TOTAL_COLOR = "#121212"   # 或者你想要的灰/黑/别的
RAW_ALPHA = 0.35
...
def _as_float_array(x):
    """Convert to 1D float array; treat missing (None) as empty array.

    NOTE: np.asarray(None, dtype=float) becomes array(nan) (scalar), which can silently
    collapse time series to length-1. We explicitly return empty for None here.
    """
    if x is None:
...
def extract_s3_series(g: Dict[str, Any]) -> S3Series:
    ...
    if ins_primary.size == 0:
        ins_primary = _as_float_array(ins.get("secondary")) + _as_float_array(ins.get("township"))
...
def plot_two_series_with_bands(
    ...
    # Safety: align X to quantile series length (handles rare length mismatches / missing keys).
    # Safety: align X to series length (median is index 2 in q_levels)
    L = min(len(X), len(qc[2]), len(qp[2]))
    if len(X) != L:
        X = X[:L]
        qc = tuple(a[:L] for a in qc)
        qp = tuple(a[:L] for a in qp)
...
def plot_insurance_dual_axis_with_bands(
    ...
    ax1.plot(X, t_mid,   linestyle="--", linewidth=1.2, alpha=RAW_ALPHA, color=TOTAL_COLOR, label="Total (raw q50)")
    ax1.plot(X, t_center, linestyle="-", linewidth=2.8, color=TOTAL_COLOR, label=f"Total ({movavg}w MA of q50)")
